...
impl AdminUserQuery {
    async fn users(&self, ctx: &Context<'_>, token: String, filter: Option<UserFilter>) -> async_graphql::Result<Vec<UserAdmin>> {
        ...
        let _ = match AdminUserServiceImpl::get_user_permissions_from_role(db.as_ref(), claims.sub, "can_read", "Pages::AdminHome").await {
            Ok(_) => {
            ...
            Err(_) => {
                ...
                match AdminUserServiceImpl::get_user_permissions_from_user(db.as_ref(), claims.sub, "can_read", "Pages::AdminHome").await {
                    Ok(_) => {
                        trace!("users: User {:?} has permission to read admin home", claims.sub);
                    },
                    Err(e) => {
